...
**convoyplan** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `convoyplan` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
convoyplan ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
convoyplan ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
convoyplan ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `convoyplan` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="convoyplan") for rec in records]   # records = your JSON output
